...
export const TeacherSubmissionView: React.FC = () => {
  ...
  const handleGradeSubmission = async () => {
    ...
    try {
      ...
      showError(t("submissions.submissionView.gradingError"), t("common.error"));
    } finally {
      setGrading(false);
    }
  };
  ...
  if (!submission) {
    return <div className="text-center p-8">{t("submissions.submissionView.notFound")}</div>;
  }
          ...
          <div className="flex flex-col items-start">
            ...
            <div className="flex items-center gap-4 mt-2">
              ...
              <div className="flex items-center gap-1 text-gray-600">
                ...
                <span>{t("submissions.submittedAt")}: {formatDateTime(submission.submittedAt)}</span>
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {images.map((imageUrl, index) => (
                <div key={index} className="relative group">
                  <img
                    src={imageUrl}
                    alt={`${t("submissions.submissionView.image")} ${index + 1}`}
        ...
        <div className="bg-white border rounded-lg p-6 h-fit">
          ...
          {submission.score !== null && (
            <div className="mb-4 p-4 bg-gray-50 rounded-lg">
              <div className="text-sm text-gray-600 mb-1">{t("submissions.submissionView.currentScore")}:</div>
              <div
              ...
              {submission.gradedAt && (
                <div className="text-xs text-gray-500 mt-1">
                  {t("submissions.submissionView.gradedAt")}: {formatDateTime(submission.gradedAt)}
                </div>
              )}
            </div>
          )}
          ...
          <div className="space-y-4">
            ...
            >
              ...
              {grading ? t("submissions.submissionView.saving") : t("submissions.submissionView.saveGrade")}
            </button>
          </div>
        </div>
      </div>
